...
apt-get update -y
apt-get -y install libncurses5-dev bc build-essential device-tree-compiler gcc-$GCCVER g++-$GCCVER
...
if [ "$headerversion" == "$kernelversion" ]; then
        ...
        echo "------------------"
        echo "INFO: you can install matching kernel and headers using the following commands:"
        echo "wget https://archive.raspberrypi.org/debian/pool/main/r/raspberrypi-firmware/raspberrypi-kernel_1.20190819-1_armhf.deb"
        echo "dpkg -i raspberrypi-kernel_1.20190819-1_armhf.deb"

        echo "wget https://archive.raspberrypi.org/debian/pool/main/r/raspberrypi-firmware/raspberrypi-kernel-headers_1.20190819-1_armhf.deb"
        echo "dpkg -i raspberrypi-kernel-headers_1.20190819-1_armhf.deb"
        echo "reboot"
        echo "------------------"

        echo -e "$ERR WARNING: kernel is outdated! use the commands from above to install matching kernel and kernel headers. Then reboot and run this script again - $NC" 1>&2
        exit 1
fi
